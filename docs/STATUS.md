...
> **the context stratum**: the token counts stop being abstract — a session's transcript becomes
> **cost-ranked blocks** and descending one shows **the actual tokens**. Cost is **residency**, not size
> (an early read is re-sent every turn after it lands), cache-weighted so the number is honest. Driving it
> corrected three of our own headline numbers — see 30a/30b, and the retraction in 31 →
> **the dictionary**: advice as **Suggestion nodes** on the blocks that cost most, ranked by estimated
> saving, offered as `:inference` so bad advice sinks rather than needing to be hidden. Three rules
> survive **because they were measured**; three plausible ones were deleted **because they weren't** →
> **the loop closes** (`hc-4s4`): `hcol ask`/`hcol fix` queue a **Request** (an intent — a fact, no
> state), `hcol run` puts an LLM on it, and **LLMTask** carries the execution. Outstanding is a question
> the *graph* answers — a Request with no task — so replay can't re-fire work. The default strategy drives
> a real **Claude Code in a tmux pane** you can attach to and take over; completion is the spawned agent's
> own **hook**, not a scraped pane — the thing hugel3 and gastown both lacked. **Proven live: an agent
> fixed a real bug from a queued request.**
>
> **Where the frontier is:** the runner works but doesn't yet survive contact — `hcol run` dies with its
> shell (a task stayed `running` though the agent had finished and the hook had reported), `stop()` is a
> bare `kill-session` against an agent that ignores SIGHUP (**it leaks panes now**), and there is no stall
> detector, so a wedge costs a full timeout instead of a visible failure. SIGWINCH was on this list and
> **measurement removed it** — a bracketed paste lands in an idle detached TUI unaided. See `hc-4s4`.
>
> **Pick up next (one of):**
> **make the runner survive** (`hc-4s4`: long-lived `hcol run`, orphan reaping, stall detector,
> `pipe-pane` as the durable output channel) · **dispatch from the UI** (the guiding star: `hcol serve`
> can already show suggestions; letting a click queue the Request is the last hop to hcol *as* the
> interface) · **the lexical index** (`hc-xj4`, unblocked now the context stratum is in: terms ranked by
> distinctiveness, anchored to real nodes) · **parked juggler
...
| Commit | Layer |
|---|---|
| `743d027` | **33a** — **the first real run: an agent fixed a real bug, and readiness was lying**. Drove `hcol run` against a **live Claude Code** instead of the Echo double: `hcol ask "calc.rb has a bug: add() subtracts instead of adding. Fix it."` produced `a - b` → `a + b` in a real repo, dispatched from a queued Request, with the agent's own hook reporting `turn`, `TOUCHES calc.rb`, `phase reviewing` and `usage` into its task-scoped log — and `finished?` returns true against that real log, so **completion-by-hook holds outside the fixture**. **SIGWINCH does not bite**: the gap predicted to hurt most turns out not to exist here (tmux 3.6a, macOS) — a bracketed paste lands in an idle, detached TUI with no resize hack, watched live. Gastown's SIGWINCH ritual solves something we don't hit; closed by measurement, not code. **Two bugs only a real run could find.** (1) `start()` **never delivered** — it spawned, accepted trust, returned; the earlier "end-to-end" test had called `deliver()` **by hand**, hiding the gap completely. Delivery moved into `poll()`, which also stops a ~10s boot blocking every other task. (2) **Readiness was `/❯/` alone — and the trust dialog draws its selection as `❯ 1. Yes, I trust this folder`.** So the instant `accept_trust` pressed Enter, the still-rendered dialog matched, the prompt was pasted onto a dying screen, delivery was marked done, and the task sat out its **entire timeout** awaiting an answer to a question nothing had been asked. Precisely gastown's bug — a marker that also appears in a state you are not in yet — walked into *immediately after reading their report of it*. The rule, now pinned both ways: scraping is fine for **readiness** (wrong costs one retry), unacceptable for **completion** (wrong costs a truncated answer). 1 spec (308). |
| `e532588` | **33** — **`hcol run`: the loop closes, and a fix is just a request**. `hcol fix` had been writing requests nobody listened for; this is the consumer, and per Charris's call it dispatches a **Request**, never "a fix". **`FixRequest` is gone as a type** — there is one `Request`, and `kind` (fix/ask/…) + `subject` are **provenance** for a human reading the column; nothing in the runner reads either. So anything that can phrase a prompt queues work (`hcol fix`, the new `hcol ask "<prompt>"`, a hook, a future rule) and the executor is open by construction rather than by a growing case statement. The generalisation split two conflated things: **Request** = the INTENT (a fact — someone asked X at T about Y; immutable, **no state**), **LLMTask** = the EXECUTION (the runner's state machine, hung off it). That split lets a request be **retried** (a second attempt is a second task, not rewritten history) — and turns *"what is outstanding?"* into a question the **graph** answers (a Request with no task referencing it), so **replay safety falls out of the fold** instead of resting on a cursor file that can drift from the log it describes. Verified: replay a log of 2 requests + their 2 tasks → outstanding is 0, so a walk cannot re-fire work. `hcol run [--echo] [--timeout N]`; `--echo` exercises the wiring without spending a token. 5 specs (307). |
| `06d866c` | **32b** — **clear the first-run trust dialog from a fact, not a guess**. `--dangerously-skip-permissions` does **not** dismiss the workspace-trust dialog — verified live in an untrusted dir rather than taken on faith (hugel3 found the same for a *different* flag; it holds for this one). In a detached pane that dialog is invisible, so a first run would wedge until the wall-clock fired. Charris asked whether a first run could be detected and bypassed with something like `yes`. **Detection: better than a heuristic** — Claude Code records `projects[<abs path>].hasTrustDialogAccepted` in `~/.claude.json`, so *"is a dialog coming?"* is a **fact we look up**. That is what makes the keystroke defensible: Enter is pressed only when the dialog is already known to be there, **confirming an expectation** rather than inferring state from pane bytes. **`yes` is the wrong tool**: it streams `y\n` at *stdin*, but a TUI reads raw keys from the tty and the dialog is a numbered menu with option 1 pre-selected — and `yes` never stops, so after the dialog cleared it would type into the prompt box forever. The bypass is **one Enter**. Also rejected: pre-seeding the config — 200KB+ of global state across 62 projects that Claude Code writes concurrently; corrupting it breaks every repo at once (gastown shipped an atomic-write fix after exactly that). One keystroke into our own pane touches nothing outside the task. 6 specs (302). |
| `579eb74` | **32a** — **skip permissions in the spawned pane (Charris's call)**. A permission prompt in a **detached** pane is invisible: the session wedges, no error, and the runner learns nothing until its wall-clock fires. hugel3 shipped the same default for the same reason. The strategy had defaulted to plain `claude`, so nothing had run bypassed before this. Taken deliberately, with the consequence stated: the spawned agent can act unattended on the repo it is pointed at — the seam already allows the mitigation, since `command:` and `root:` are injectable, so an untrusted task can be pointed at a worktree. Recorded as a **spec**, not just a constant: changing a safety trade-off should take saying so out loud. |
| `c93d904` | **32** — **LLMTaskRunner (`hc-4s4`): fire an LLM request, get the answer back as a node**. The consumer `hcol fix` was already writing requests for, and the first step toward hcol as an interface for LLM-backed agentic development. **Seam first**: `LLMTaskRunner(strategy:)` with start/poll/stop, and **`Strategies::Echo`** as the double — so the whole lifecycle (pending → running → done/failed, timeouts, a strategy throwing mid-flight) is specified with **no LLM, no network, no tmux**. A strategy that can only be tested by talking to a model can't be tested. Out of process, per layer 17: concurrency lives between processes, the file is the shared state, no mutex. A task is one digest-keyed `LLMTask` re-emitted per state (the TestRun pattern) — ◌ → ◐ → ✓ flips **in place**; the node exists **before** the work does, because a task you can only see once it finished is a receipt, not a view of work. `DISPATCHED` is `:agent` evidence — a model's answer is its account, not ground truth. **Prior art settled the tmux choices, and hugel3/gastown made OPPOSITE ones** (hugel3's design doc lists gastown's approach in a *Rejected* table). We follow hugel3: **input** = `load-buffer` + `paste-buffer -p -d` + a separate `send-keys Enter` (their note: keystroke injection "is the single biggest reason tmux+TUI looks flaky"; gastown took send-keys and paid ~1100 lines — 512-byte chunking against a 4096 TTY line-discipline limit, a 600ms sleep so readline doesn't read ESC as a meta-prefix, a Rewind-menu detector that re-sends the whole message, a per-vendor Escape-poison matrix). Probed live **before** reading any of it and independently landed on the same commands. **Completion = an event, and neither project had it**: gastown greps the pane for the literal `"esc to interrupt"` (undocumented, cosmetic, and it lies often enough to need a two-poll debounce); hugel3 refused to scrape but its fixed sentinel fires the moment you *paste a prompt containing it*. We need neither — the spawned agent carries our bridge hook, so launched with `-e HCOL_BRIDGE_LOG` at a **task-scoped log** it reports itself, and the harness-enforced Stop hook is the signal. The research agent independently called hooks "the best available signal today that neither used". The task-scoped log also dodges `adopt_spine`, which would have a second agent hijack the shared log's Session node. Fixed 200×50 geometry; panes addressed `session:window.pane`, never bare (gastown: bare targets follow whatever pane the human last clicked). 7 specs (295). |
| `4d1cd1c` | **31** — **the dictionary (`hc-bnh`): suggestions as nodes, and a request an agent can act on**. Advice attached to the blocks that cost the most. **What measurement did to it: the pattern it was built around died.** "The same content gets re-read" was reported at **18.3%** of all cost, headlined by *"one PNG read 8 times = 12% of everything"*. Both were artifacts of my own analysis — grouping ran **across sessions** (a file read once in each of eight sessions is eight necessary reads, not seven wasted ones → 13.3%), and the fingerprint rendered a block to its **text**, which an image has none of, so eight **different** screenshots collided into one bogus group (→ **1.3%**). Each PNG was read once, legitimately, while writing the README; what survives at 1.3% is boilerplate "file has been updated" acks. **Rule deleted**, by the same test that rejected error-results (0.1%) and imported cache-thrash (its ≥12-parallel-call precondition has never once occurred here; our max ever is 3). A rule that fires on nothing teaches you to ignore the panel. The fingerprint **fix** was kept — it was a real bug mis-grouping our largest blocks. Three rules survive with their measured shares: `early_large_read` **13.2%** (the STATUS.md shape), `long_thinking` **20.4%** (shippable only because 30b proved thinking is retained), `unpiped_output` **1.0%**. Suggestions are **nodes** (`SUGGESTS` from the transcript, `ABOUT` the block) — verified live: suggestion → block → `READS` → the real file. An `advice` facet ranks by saving in **cost-equivalent** (wire would overstate every recommendation ~8× — a lie in the user's favour, the worse kind). Advice is `:inference`, so bad advice **sinks** rather than needing hiding. `hcol fix` emits the log's first **intent** and stops: dispatch belongs to a consumer with its own cursor, because the log is replayed on every walk and a read-model must not dispatch code changes behind your back. **Specs pin the rejections**, so the comments can't drift back. 9 specs (288). |
| `9bd6b71` | **30b** — **a turn is a round-trip: the 2.1× inflation, and thinking is retained**. Chasing layer 30's open question — do prior-turn thinking blocks stay in the re-sent context? — found a counting bug under **both** the answer and the shipped headline. **Claude Code writes ONE API response as SEVERAL transcript entries** (one per content block), each repeating that response's usage **and** its message id: 539 entries carrying only **247** distinct message ids; 4133 entry-"turns" for **1931** real round-trips. So the hook's `report_usage` summed usage **per entry**, multiplying every turn's totals by however many blocks the reply happened to have — **the `9.2M in → 32k out` headline shipped in layer 27 had been overstating by ~2.1× ever since** — and the provider inflated every block's residency identically. Both now key on `message.id`. Corrected: **337.1M** wire (reported 685.4M), **39.0M** cost-equivalent (82.6M), 99% of wire is cache reads. This session's own view drops from a claimed 180× re-read to an honest **105×**. Category *shares* survived (the inflation was near-uniform) but every absolute number in the first pass was wrong. **The answer: thinking IS retained.** Ground truth is the observed context size per round-trip (`input + cache_read + cache_creation`); model A (retained) vs B (stripped) fitted against the growth between consecutive round-trips over the **361** pairs where >500 tok of thinking was emitted — **A wins 361/361**, median error 431 tok vs 1749. So the cost is real and `hc-bnh`'s held-back entry could ship. Note what settled it: **measuring**, not the literature — the web research had nothing either way. 1 spec (279). |
| `02282e7` | **30a** — **cost-equivalent, not wire volume: the cache multipliers land**. Research into published cache economics caught the model shipped an hour earlier **overstating the bill ~8×**. Anthropic prices a cache **read** at **0.1×** base input and a **write** at **1.25×** (5-min TTL). The model counted every resident token at face value, reporting "≈7.7M billed" for a block worth ~10× less. Measured across our own 12 transcripts (4102 billed turns): **685.4M wire, but 98% of it cache reads → 82.6M cost-equivalent, 12% of the headline**; fresh input across all 12 sessions is 0.2M, a rounding error. Cost is now `tokens × (1.25 + 0.1 × resident)`, and the facet reports **both** — wire (which carries the re-read story) and cost-equivalent (what you actually paid). Either alone lies: wire invites panic about a bill nobody paid; cost alone hides the ~180× re-read. It also **sharpened the design's central claim** rather than rescaling it: residency still dominates, but there is a **crossover** set by the write premium — a block must stay resident **~19 turns** to outcost one 2.5× its size, **~113** to outcost one 10×. Below the line the bigger block genuinely is dearer. The spec asserting "early small beats late large" had been written *below* the line and correctly started failing; it now pins the crossover instead of a happy accident. Also settles `hc-f47`'s headline question. 1 spec (278). |
...
agent_bridge.rb    the REAL producer (hc-gzj): a neutral agent vocabulary (edit/phase/test/log/done
                   + turn/usage/transcript/task/request)
...
providers/transcript.rb
                   the CONTEXT stratum (hc-33x): a Claude Code transcript -> cost-ranked ContextBlock
                   nodes. Cost = tokens x (1.25 + 0.1 x resident) — cache-weighted, because a re-read
                   bills at 0.1x and wire volume overstates the bill ~8x. A turn is one API ROUND-TRIP
                   (message.id), not one transcript entry — entries repeat a response's usage, which
                   inflated everything 2.1x until 30b. Text NEVER enters the graph: a line pointer +
                   read-back on demand (the content-tab precedent), so a 2.8MB transcript costs KBs.
providers/context_advice.rb
                   the DICTIONARY (hc-bnh): rules -> Suggestion nodes on the blocks that cost most,
                   ranked by saving, emitted as :inference (advice is a claim; bad advice sinks). Every
                   rule carries its measured share; three plausible rules are deliberately ABSENT and
                   specced as such, because measurement said they fire on nothing.
llm_task_runner.rb the RUNNER (hc-4s4): Request (intent, immutable) -> LLMTask (execution, a state
                   machine). `outstanding` asks the GRAPH which requests have no task, so replay can't
                   re-fire work. Strategy seam: Echo (double — the lifecycle is testable with no LLM)
                   / TmuxClaudeCode (default).
strategies/tmux_claude_code.rb
                   drives a real Claude Code in a tmux pane you can attach to. Input = bracketed paste
                   from a file (hugel3's path; send-keys mangles multi-line). Completion = the spawned
                   agent's own Stop hook writing a task-scoped log — an EVENT, which neither hugel3 nor
                   gastown had. Scraping is used ONLY for readiness (wrong costs a retry) and never for
                   completion (wrong costs a truncated answer).
